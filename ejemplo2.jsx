...
const Users = () => {
  ...
  const handleEditPolygons = (polygon) => {
    setPolygonToEdit(polygon); // Establecer el poligono a editar
    setView("edit");
  };
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {view === "edit" && (
        <EditSector
          sector={sectorToEdit}
          onUserUpdated={(updatedSector) => {
            const updatedSector = polygon.map((sector) =>
              sector.id === updatedSector.id ? updatedSector : sector
            );
            localStorage.setItem("", JSON.stringify(updatedSector));
            handleUserUpdated(updatedSector);
          }}
          onBack={() => setView("list")}
        />
      )} 
